...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const path = getFunctionSubpath(url, "swipe");
    ...
    if (req.method === "POST" && path === "") {
      ...
      if (rpcError) {
        ...
        const { error: insertErr } = await supabaseAdmin.from("swipes").upsert(
          {
            swiper_id: user.id,
      ...
      if (body.direction === "like") {
        ...
        if (reverseSwipe) {
          ...
          const { data: match } = await supabaseAdmin
            ...
            .or(
              `and(user_a.eq.${user.id},user_b.eq.${body.target_id}),and(user_a.eq.${body.target_id},user_b.eq.${user.id})`,
            )
            .maybeSingle();
