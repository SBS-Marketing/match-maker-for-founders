...
async function computeMatchesForUser(
  ...
  const { data: myProfile, error: myErr } = await supabaseAdmin
    .from('profiles')
    .select('id, display_name, bio, skills, looking_for, vision, lat, lng, city, max_distance_km, industry, role, stage')
    .eq('id', userId)
    .single()
  ...
  for (const candidate of candidates as FounderProfile[]) {
    ...
    // Location
    const { score: locScore, distanceKm } = computeLocationScore(myProfile as FounderProfile, candidate)
    ...
    const explanation: Record<string, unknown> = {
      ...
      common_skills: (myProfile.skills ?? []).filter((s: string) => (candidate.skills ?? []).includes(s)),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const path = url.pathname.replace(/^\/matching\/?/, '')
    ...
    if (req.method === 'GET' && path === 'recommendations') {
      ...
      const { data: recs, error } = await supabaseAdmin
        ...
        .select(`
          *,
          target:profiles!match_results_target_id_fkey(id, display_name, photo_url, city, role, stage, industry, skills, bio)
        `)
        .eq('user_id', user.id)
      ...
      return new Response(JSON.stringify({ success: true, recommendations: recs }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
