    ...
    <div className="mx-auto flex h-[calc(100svh-10rem)] max-w-5xl flex-col overflow-hidden px-3 pt-3 sm:h-auto sm:px-6 sm:pt-10 sm:pb-24">
      <div className="flex shrink-0 items-center gap-2">
        ...
        <span className="eyebrow">
          {s.label} · {partners.length || s.count} kuratiert
        </span>
      </div>
          ...
          <article key={partner.slug} className="glass-pane p-5">
              ...
              <div className="flex min-w-0 items-start gap-3">
                {partner.logoUrl && (
                  <img
                    ...
                    alt=""
                    loading="lazy"
                    className="h-11 w-11 shrink-0 rounded-xl border border-[rgba(21,20,15,0.08)] bg-white object-contain p-1"
                  />
                )}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {partner.specialties.slice(0, 4).map((specialty) => (
                <span
                  key={specialty.label}
                  className="rounded-full bg-[var(--paper)] px-2.5 py-1 text-[11px] font-medium text-[var(--ink-soft)]"
                >
                  {specialty.label}
                </span>
              ))}
